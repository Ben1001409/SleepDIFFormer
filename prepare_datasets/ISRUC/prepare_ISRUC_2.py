...
dir_path="/home/tester1/YT/Ben/SleepDiFFormer/data/datasets2/ISRUC_extracted/group2"

seq_dir="/home/tester1/YT/Ben/SleepDiFFormer/data/datasets/GSS_datasets/ISRUC/seq"

label_dir="/home/tester1/YT/Ben/SleepDiFFormer/data/datasets/GSS_datasets/ISRUC/labels"
...
for i in range(1, 9):
    ...
    psg_f_names.append(f'{dir_path}/{numstr}/{numstr}/1/1.rec')

    label_f_names.append(f'{dir_path}/{numstr}/{numstr}/1/1_1.txt')

    psg_f_names.append(f'{dir_path}/{numstr}/{numstr}/2/2.rec')
    label_f_names.append(f'{dir_path}/{numstr}/{numstr}/2/2_1.txt')
...
for psg_f_name, label_f_name in tqdm(psg_label_f_pairs):
    ...
    if not os.path.isdir(f'{seq_dir}/ISRUC-group2-{str(n)}'):
        os.makedirs(f'{seq_dir}/ISRUC-group2-{str(n)}')
    for seq in epochs_seq:
        seq_name = f'{seq_dir}/ISRUC-group2-{str(n)}/ISRUC-group2-{str(n)}-{str(num_seqs)}.npy'
        with open(seq_name, 'wb') as f:
            np.save(f, seq)
        num_seqs += 1

    if not os.path.isdir(f'{label_dir}/ISRUC-group2-{str(n)}'):
        os.makedirs(f'{label_dir}/ISRUC-group2-{str(n)}')
    for label in labels_seq:
        label_name = f'{label_dir}/ISRUC-group2-{str(n)}/ISRUC-group2-{str(n)}-{str(num_labels)}.npy'
